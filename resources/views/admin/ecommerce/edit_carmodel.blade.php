...
@section('content')
    <form action="{{ route('admin.ecommerce.update_carmodel', ['id' => $carmodel->id]) }}" method="post"
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="tab_detail">
                                      ...
                                      <div class="form-group mb-3">
                                                <label for="car_model" class="control-label required" aria-required="true">Car model</label>
                                                <input class="form-control" placeholder="Car model" data-counter="120" v-pre=""
                                                       name="car_model" type="text" id="car_model" value="{{ old('car_model', $carmodel->car_model) }}" aria-invalid="false"
                                                       aria-describedby="car_model-error">
                                                @if ($errors->has('car_model'))
                                                    <span id="car_model-error" class="text-danger">{{ $errors->first('car_model') }}</span>
                                                @endif
                                            </div>
                                    
                                            <div class="form-group mb-3">
                                                <label for="brand_id" class="control-label required" aria-required="true">Brand</label>
                                                <select class="form-select" aria-label="Default select example" name="brand_id">
                                                    <option value="" disabled {{ old('brand_id', $carmodel->brand_id) ? '' : 'selected' }}>Select brand</option>
                                                ...
                                                @if ($errors->has('brand_id'))
                                                    <span id="brand_id-error" class="text-danger">{{ $errors->first('brand_id') }}</span>
                                                @endif
                                            </div>
                                    <!--<div class="form-group mb-3">-->
                                    ...
                                    <div class="ui-select-wrapper form-group ">
                                        <select class="form-control ui-select" v-pre="" id="status"
                                            ...
                                            <option value="published"
                                                {{ $carmodel->status == 'published' ? 'selected' : '' }}>
                                                Published</option>
                                            <option value="draft" {{ $carmodel->status == 'draft' ? 'selected' : '' }}>Draft
                                            </option>
                                            <option value="pending" {{ $carmodel->status == 'pending' ? 'selected' : '' }}>
                                                Pending
                                            </option>
                                        </select>
